...
func NewStaticSourceFromImage(ctx context.Context, resolver remotes.Resolver, ref string) (*ImageLayerSource, error) {
	...
	for _, name := range parsedEnvs.keys {
		...
		if strings.HasPrefix(name, "GITPOD_APPEND_ENV_") {
			name = strings.TrimPrefix(name, "GITPOD_APPEND_ENV_")
			if name == "" || value == "" {
				continue
			}
			envs = append(envs, newAppendEnvModifier(name, value))
		} else if strings.HasPrefix(name, "GITPOD_PREPEND_ENV_") {
			name = strings.TrimPrefix(name, "GITPOD_PREPEND_ENV_")
			if name == "" || value == "" {
				continue
			}
			envs = append(envs, newPrependEnvModifier(name, value))
		} else if strings.HasPrefix(name, "GITPOD_SET_ENV_") {
			name = strings.TrimPrefix(name, "GITPOD_SET_ENV_")
			if name == "" {
				continue
			}
